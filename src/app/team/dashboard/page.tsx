...
import { getTeamDashboardData, resubmitRegistrationById, startNewTeamRegistration } from '@/lib/registration/actions';
...
export default async function TeamDashboardPage() {
  ...
  async function handleResubmit(formData: FormData) {
    ...
    await resubmitRegistrationById(regId);
    ...
    <>
      ...
      <main className="relative z-10 mx-auto flex max-w-3xl flex-col items-center px-4 pb-8 pt-20 md:px-10">
        ...
        <div className="flex w-full flex-col gap-6">
          <form action={startNewTeamRegistration} className="self-start">
            <button
              type="submit"
            ...
            >
              <Plus size={20} />
              <span className="font-sans text-body-md font-medium">Register another team</span>
            </button>
          </form>
          ...
          {teams.map(({ team, registration }) => {
            ...
            const isPhase1Draft = registration?.phase === 'PHASE_1' && registration?.status === 'DRAFT';
            const isPhase1Submitted = registration?.phase === 'PHASE_1' && registration?.status === 'SUBMITTED';
            const isSlotApproved = registration?.phase === 'PHASE_2';
            const isPhase2Draft = isSlotApproved && registration?.status === 'DRAFT';
            const isPhase2Submitted = isSlotApproved && ['SUBMITTED', 'UNDER_REVIEW', 'RESUBMITTED'].includes(registration.status);
            const isApproved = registration?.status === 'APPROVED';
            const isRejected = registration?.status === 'REJECTED';
            const isChangesRequested = registration?.status === 'CHANGES_REQUESTED';

            const isSubmitted = registration?.status === 'SUBMITTED' || isPhase2Submitted || isChangesRequested || isApproved || isRejected;
              ...
              >
                <div
                  className={`absolute left-0 right-0 top-0 h-[1px] bg-gradient-to-r from-transparent ${
                    isSubmitted ? 'via-primary-container/50' : 'via-white/20'
                  } to-transparent`}
                />
                ...
                <div className="flex items-start justify-between">
                  <div className="flex flex-col gap-2">
                    <h2 className="font-sans text-2xl font-semibold text-on-surface">
                      {team.name === 'Draft Team' ? 'Untitled team' : team.name}
                    </h2>
                      ...
                      >
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${
                            isSubmitted ? 'bg-primary-container shadow-[0_0_15px_rgba(0,240,255,0.3)]' : 'bg-outline-variant'
                          }`}
                        />
                        {registration?.status ?? 'DRAFT'}
                      </span>
                      <span className="font-mono text-xs text-outline">
                        {registration?.phase === 'PHASE_1' ? 'Tournament Participation' : 'Team Details Submission'}
                        {registration?.division && ` · ${registration.division === 'MENS' ? "Men's" : "Women's"}`}
                      </span>
                    </div>
                    {team.institutionType && (
                      <div className="mt-2 flex items-center gap-4">
                        ...
                        {registration?.division && (
                          <span className="font-mono text-xs uppercase tracking-wider text-outline">
                            {registration.division === 'MENS' ? "Men's Division" : "Women's Division"}
                          </span>
                        )}
                      </div>
                    )}
                  ...
                  <div className="flex flex-col items-end">
                    <span className="font-mono text-xs uppercase tracking-wider text-outline">Players</span>
                    <span className="font-sans text-2xl font-semibold text-on-surface">{players.length}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {isSlotApproved && isPhase2Draft && (
                    <>
                      <div className="rounded-lg border border-green-500/30 bg-green-500/10 p-3 text-center font-sans text-body-md text-green-400">
                        <CheckCircle size={14} className="mr-1 inline" /> Slot approved!
                      </div>
                      <Link
                        href="/team/register/officials"
                        className="w-full rounded-lg bg-primary-container py-3 text-center font-sans text-body-md font-semibold text-on-primary-container shadow-[0_0_15px_rgba(0,240,255,0.3)] transition-colors hover:bg-primary"
                      >
                  ...
                  {isChangesRequested && (
                    <form action={handleResubmit} className="flex flex-col gap-3">
                      <input type="hidden" name="registrationId" value={registration.id} />
                      <div className="rounded-lg border border-tertiary/30 bg-tertiary/10 p-3 font-sans text-body-md text-tertiary">
                        <AlertTriangle size={14} className="mr-1 inline" /> Changes requested.
                      </div>
                      <button
                        type="submit"
                        className="w-full rounded-lg bg-primary-container py-3 text-center font-sans text-body-md font-semibold text-on-primary-container shadow-[0_0_15px_rgba(0,240,255,0.3)] transition-colors hover:bg-primary"
